  ...
  for (int h = 0; h != kernel_height; ++h) {
    ...
    for (int w = 0; w != kernel_width; ++w) {
      ...
      for (int c = 0; c < input_channels; c += IN_CHANNEL_GROUP_SIZE) {
        half8 in_val = vload8(0, &input[in_val_base_idx + c]);
        half8 kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s0 += dot(in_val.hi, kernel_val.hi);
        out_val.s0 += dot(in_val.lo, kernel_val.lo);

        kernel_val_idx += IN_CHANNEL_GROUP_SIZE;
        kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s1 += dot(in_val.hi, kernel_val.hi);
        out_val.s1 += dot(in_val.lo, kernel_val.lo);

        kernel_val_idx += IN_CHANNEL_GROUP_SIZE;
        kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s2 += dot(in_val.hi, kernel_val.hi);
        out_val.s2 += dot(in_val.lo, kernel_val.lo);

        kernel_val_idx += IN_CHANNEL_GROUP_SIZE;
        kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s3 += dot(in_val.hi, kernel_val.hi);
        out_val.s3 += dot(in_val.lo, kernel_val.lo);

        kernel_val_idx += IN_CHANNEL_GROUP_SIZE;
        kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s4 += dot(in_val.hi, kernel_val.hi);
        out_val.s4 += dot(in_val.lo, kernel_val.lo);

        kernel_val_idx += IN_CHANNEL_GROUP_SIZE;
        kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s5 += dot(in_val.hi, kernel_val.hi);
        out_val.s5 += dot(in_val.lo, kernel_val.lo);

        kernel_val_idx += IN_CHANNEL_GROUP_SIZE;
        kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s6 += dot(in_val.hi, kernel_val.hi);
        out_val.s6 += dot(in_val.lo, kernel_val.lo);

        kernel_val_idx += IN_CHANNEL_GROUP_SIZE;
        kernel_val = vload8(0, &weights[kernel_val_idx]);
        out_val.s7 += dot(in_val.hi, kernel_val.hi);
        out_val.s7 += dot(in_val.lo, kernel_val.lo);
