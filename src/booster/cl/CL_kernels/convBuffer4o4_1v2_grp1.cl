...
__kernel void convolution(__global const half* restrict input,   /* [h, w, c] */
  ...
  half4 in_val0, in_val1, kernel_val;
  half4 out_val0 = vload4(0, &bias[out_channel_idx]);
  half4 out_val1 = out_val0;
  for (int h = 0; h != kernel_height; ++h) {
    ...
    for (int w = 0; w != kernel_width; ++w) {
      ...
      for (int c = 0; c < input_channels; c += 4) {
        in_val0 = vload4(0, &input[in_val_beg0 + c]);
        in_val1 = vload4(0, &input[in_val_beg1 + c]);
...
#undef LOAD_KERNEL_AND_CALC
  ...
  if (use_relu) {
    out_val0 = fmax(out_val0, (half4)0);
    out_val1 = fmax(out_val1, (half4)0);
  }

  int out_val_idx = mad24(mad24(out_height_idx, output_width, out_width_idx), output_channels, out_channel_idx);
  vstore4(out_val0, 0, &output[out_val_idx]);
  if (out_width_idx + 1 < output_width) {
    vstore4(out_val1, 0, &output[out_val_idx + output_channels]);
  }
}
