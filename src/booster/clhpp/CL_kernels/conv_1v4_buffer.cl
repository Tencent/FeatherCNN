...
__kernel void convolution(__global const DATA_TYPE* restrict input,   /* [ih, iw, ic] */
  ...
  for (int h = 0; h != kernel_height; ++h) {
    ...
    for (int w = 0; w != kernel_width; ++w) {
      ...
      if (in_width_idx2 >= input_width) in_width_idx2 = input_width * 2 - 2 - in_width_idx2;
      int in_width_idx3 = in_width_beg3 + w;
      if (in_width_idx3 < 0) in_width_idx3 = -in_width_idx3;
      if (in_width_idx3 >= input_width) in_width_idx3 = input_width * 2 - 2 - in_width_idx3;
      ...
      const int in_val_beg2 = mad24(in_width_idx2, input_channels, in_val_base_width_idx);
      const int in_val_beg3 = mad24(in_width_idx3, input_channels, in_val_base_width_idx);
      for (int c = 0; c < input_channels; c += N) {
        in_val0 = VLOADN(0, &input[in_val_beg0 + c]);
        in_val1 = VLOADN(0, &input[in_val_beg1 + c]);
        in_val2 = VLOADN(0, &input[in_val_beg2 + c]);
        in_val3 = VLOADN(0, &input[in_val_beg3 + c]);
...
#undef LOAD_KERNEL_AND_CALC
  ...
  out_val_idx += output_channels;
  if (out_width_idx + 1 < output_width) {
    VSTOREN(out_val1, 0, &output[out_val_idx]);
    out_val_idx += output_channels;
    if (out_width_idx + 2 < output_width) {
      VSTOREN(out_val2, 0, &output[out_val_idx]);
      out_val_idx += output_channels;
      if (out_width_idx + 3 < output_width) {
        VSTOREN(out_val3, 0, &output[out_val_idx]);
      }
    }
  }
}
