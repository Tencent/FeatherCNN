...
from itl.layers.caffe_pb2 import PoolingParameter
...
class MaxPool2d(Layer):
    ...
    padding : tuple of int
        (height, width) for padding method.
    name : str
        A unique layer name.
    ...
    # @deprecated_alias(net='prev_layer', end_support_version=1.9)  # TODO remove this line for the 1.9 release
    def __init__(self, prev_layer, filter_size=(2, 2), strides=(2, 2), padding=(0, 0), name='maxpool2d'):
...
#        pdb.set_trace()
        ...
        ksize4d = [1, filter_size[0], filter_size[1], 1]
        strides4d = [1, strides[0], strides[1], 1]
        padding4d = [[0, 0], [padding[0], padding[0]], [padding[1], padding[1]], [0, 0]]

        if padding != (0, 0):
            self.outputs = tf.pad(self.inputs, padding4d)
            padding_string = 'VALID'
        else:
            self.outputs = self.inputs
            padding_string = 'SAME'

        self.outputs = tf.nn.max_pool(self.outputs, ksize4d, strides4d, padding=padding_string, name=name)
        self._add_layers(self.outputs)

    def to_caffe_prototxt(self, recursive=True):
        pooling_layer = self.create_caffe_layer() 

        pooling_layer.name = self.name
        pooling_layer.type = 'Pooling'
        self.append_bottom_from_inputs(pooling_layer)
        self.append_top_from_outputs(pooling_layer)
        
        pooling_layer.pooling_param.pool = PoolingParameter.MAX
        pooling_layer.pooling_param.pad_h = self.padding[0]
        pooling_layer.pooling_param.pad_w = self.padding[1]
        pooling_layer.pooling_param.kernel_h = self.filter_size[0]
        pooling_layer.pooling_param.kernel_w = self.filter_size[1]
        pooling_layer.pooling_param.stride_h = self.strides[0]
        pooling_layer.pooling_param.stride_w = self.strides[1]
...
#    def __init__(self, prev_layer, filter_size=(3, 3), strides=(2, 2), padding='SAME', name='meanpool2d'):
    def __init__(self, prev_layer, filter_size=(2, 2), strides=(2, 2), padding=(0,0), name='meanpool2d'):
        ...
        self.outputs = tf.nn.avg_pool(self.outputs, ksize4d, strides4d, padding=padding_string, name=name)
...
#        )
    ...
    def to_caffe_prototxt(self, recursive=True):
        ...
        pooling_layer.pooling_param.pool = PoolingParameter.AVE
        pooling_layer.pooling_param.pad_h = self.padding[0]
        pooling_layer.pooling_param.pad_w = self.padding[1]
        pooling_layer.pooling_param.kernel_h = self.filter_size[0]
        pooling_layer.pooling_param.kernel_w = self.filter_size[1]
