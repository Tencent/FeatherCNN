...
class DropoutLayer(Layer):
    ...
    Examples
    --------
    Method 1: Using ``all_drop`` see `tutorial_mlp_dropout1.py <https://github.com/itl/itl/blob/master/example/tutorial_mlp_dropout1.py>`__
    ...
    Method 2: Without using ``all_drop`` see `tutorial_mlp_dropout2.py <https://github.com/itl/itl/blob/master/example/tutorial_mlp_dropout2.py>`__
    ...
    def to_caffe_prototxt(self):
        drop_layer = self.create_caffe_layer()
        
        drop_layer.name = self.name
        drop_layer.type = 'Dropout'
        self.append_bottom_from_inputs(drop_layer)
        self.append_top_from_outputs(drop_layer)

        drop_layer.dropout_param.dropout_ratio =  self.keep
