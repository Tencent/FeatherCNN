...
from itl.layers.caffe_pb2 import EltwiseParameter
...
class ConcatLayer(Layer):
    ...
    def to_caffe_prototxt(self):
        concat_layer = self.create_caffe_layer()

        concat_layer.name = self.name
        concat_layer.type = 'Concat'
        self.append_bottom_from_inputs(concat_layer)
        self.append_top_from_outputs(concat_layer)
...
class ElementwiseLayer(Layer):
    ...
    def to_caffe_prototxt(self):
        eltwise_layer = self.create_caffe_layer()

        eltwise_layer.name = self.name
        eltwise_layer.type = 'Eltwise'

        self.append_bottom_from_inputs(eltwise_layer)
        self.append_top_from_outputs(eltwise_layer)

        if self.combine_fn == tf.maximum:
            eltwise_layer.eltwise_param.operation = EltwiseParameter.MAX
        elif self.combine_fn == tf.add:
            eltwise_layer.eltwise_param.operation = EltwiseParameter.SUM
        elif self.combine_fn == tf.multiply:
            eltwise_layer.eltwise_param.operation = EltwiseParameter.PROD
