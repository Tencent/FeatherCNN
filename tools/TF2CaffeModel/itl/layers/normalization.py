...
from itl.layers.core import TF_GRAPHKEYS_VARIABLES
...
class BatchNormLayer(Layer):
    ...
    def __init__(
            ...
            moving_variance_init=tf.constant_initializer(1.0),
            lr=(1, 1), # lr of beta, gamma, moving_mean, moving_variance
        ...
        with tf.variable_scope(name):
            ...
            if beta_init:
                ...
                variables.append(self.beta)
                self.add_lr_to_gradient_operations(self.lr[0], self.beta)
            else:
                self.beta = None
            ...
            if gamma_init:
                ...
                variables.append(self.gamma)
                self.add_lr_to_gradient_operations(self.lr[1], self.gamma)
            else:
                self.gamma = None
            ...
            self.moving_variance = tf.get_variable(
                'moving_variance',
                params_shape,
                initializer=moving_variance_init,
                dtype=LayersConfig.tf_dtype,
                trainable=False,
            )
            ...
            self.outputs = self._apply_activation(
                tf.nn.batch_normalization(self.inputs, mean, var, self.beta, self.gamma, epsilon)
            )

            variables.extend([self.moving_mean, self.moving_variance])
            #self.add_lr_to_gradient_operations(self.lr[2], self.moving_mean)
            #self.add_lr_to_gradient_operations(self.lr[3], self.moving_variance)

        self._add_layers(self.outputs)
        self._add_params(variables)

    def to_caffe_prototxt(self):
        batchnorm_layer = self.create_caffe_layer()

        batchnorm_layer.name = self.name + '/BatchNorm'
        batchnorm_layer.type = 'BatchNorm'
        self.append_bottom_from_inputs(batchnorm_layer)
        self.append_top_from_outputs(batchnorm_layer)

        if self.moving_mean is not None:
            param = batchnorm_layer.param.add()
            param.lr_mult = 0
            param.decay_mult = 0
        ...
        batchnorm_layer.batch_norm_param.use_global_stats = True
        batchnorm_layer.batch_norm_param.eps = self.epsilon

        # blobs
        if self.moving_mean is not None:
            blob = batchnorm_layer.blobs.add()
            name = [ord(c) for c in self.moving_mean.name]
            blob.data.extend(name)

        if self.moving_variance is not None:
            blob = batchnorm_layer.blobs.add()
            name = [ord(c) for c in self.moving_variance.name]
            blob.data.extend(name)
        ...
        scale_layer = self.create_caffe_layer()

        scale_layer.name = self.name + '/Scale'
        scale_layer.type = 'Scale'
        self.append_repeated_blob_names(scale_layer.bottom, self.outputs)
        self.append_repeated_blob_names(scale_layer.top, self.outputs)

        if self.beta is not None:
            param = scale_layer.param.add()
            param.lr_mult = self.lr[0]
            param.decay_mult = 0

        if self.gamma is not None:
            param = scale_layer.param.add()
            param.lr_mult = self.lr[1]
            param.decay_mult = 0
        ...
        # blobs
        if self.gamma is not None:
            blob = scale_layer.blobs.add()
            name = [ord(c) for c in self.gamma.name]
            blob.data.extend(name)
            
        if self.beta is not None:
            blob = scale_layer.blobs.add()
            name = [ord(c) for c in self.beta.name]
            blob.data.extend(name)
