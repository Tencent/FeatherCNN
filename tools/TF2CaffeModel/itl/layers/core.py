...
from itl.layers.utils import list_remove_repeat

from itl import logging

# from itl.decorators import deprecated_alias
# from itl.decorators import protected_method
# from itl.decorators import private_method

from itl.layers.caffe_pb2 import NetParameter
...
def l2_regularizer(weight=1.0, scope=None):
    ...
    def regularizer(tensor):
        with tf.name_scope('regularizer_l2'):
...
class Layer(object):
    ...
    def __init__(self, prev_layer, act=None, name=None, *args, **kwargs):
        ...
        self.add_to_all_itls()
        self.prev_layer = prev_layer if isinstance(prev_layer, Layer) else None
    ...
    def get_gradient_operations(self):
        return type(self).gradient_operations

    def add_lr_to_gradient_operations(self, lr, variable):
        # if not self.get_gradient_operations().has_key(lr):
        if not lr in self.get_gradient_operations():
            self.get_gradient_operations()[lr] = []
        self.get_gradient_operations()[lr].append(variable)

    #region caffe prototxt support

    caffe_model = NetParameter()
    all_itls = []

    def get_caffe_model(self):
        return type(self).caffe_model

    def get_all_itls(self):
        return type(self).all_itls

    def add_to_all_itls(self):
        self.get_all_itls().append(self)

    def dump_all_layers_to_caffe_prototxt(self):
        for layer in self.get_all_itls():
            layer.to_caffe_prototxt()

    def get_Gflops_a_size(self):
        Gflops = 0
        size = 0
        Gflops_list=[]
        for layer in self.get_all_itls():
            oGflops,oSize = layer.get_layers_flops_a_size()
            oGflops =int(oGflops)
            oSize = int(oSize)
            Gflops += oGflops
            Gflops_list.append([layer.name,oGflops])
            size += oSize
        print("------------show-param------------\n")
        print("------------Gflops:"+str(Gflops*1.0/1000000000)+" ---- "+str(Gflops)+"-----\n")
        print("------------size:"+str(size/1.0*4/1024/1024)+"m-----\n")
        print("percent show:\n")
        for one in Gflops_list:
            if one[1]==0:
                continue
            print("name:",one[0],"num",one[1],"percent:",one[1]*1.0/Gflops,"\n")


    def get_layers_flops_a_size(self):
        return 0, 0

    def create_caffe_layer(self):
        return type(self).caffe_model.layer.add()

    def to_caffe_prototxt(self):
        caffe_layer = self.create_caffe_layer()

        caffe_layer.name = self.name
        caffe_layer.type = 'None'

        self.append_bottom_from_inputs(caffe_layer)
        if self.outputs is not None:
            self.append_top_from_outputs(caffe_layer)
        else:
            self.append_repeated_blob_names(caffe_layer.top, self.inputs)

        self.add_activation_layer(self.act)

    def add_activation_layer(self):
        if self.act is not None:
            if self.act == tf.nn.relu:
                # ReLU
                relu_layer = self.create_caffe_layer()

                relu_layer.name = self.name + '/relu'
                relu_layer.type = 'ReLU'
                if self.outputs is not None:
                    self.append_repeated_blob_names(relu_layer.bottom, self.outputs)
                    self.append_repeated_blob_names(relu_layer.top, self.outputs)
                else:
                    self.append_repeated_blob_names(relu_layer.bottom, self.inputs)
                    self.append_repeated_blob_names(relu_layer.top, self.inputs)
            elif self.act == tf.nn.relu6:
                # ReLU6
                relu6_layer = self.create_caffe_layer()

                relu6_layer.name = self.name + '/relu6'
                relu6_layer.type = 'ReLU6'
                if self.outputs is not None:
                    self.append_repeated_blob_names(relu6_layer.bottom, self.outputs)
                    self.append_repeated_blob_names(relu6_layer.top, self.outputs)
                else:
                    self.append_repeated_blob_names(relu6_layer.bottom, self.inputs)
                    self.append_repeated_blob_names(relu6_layer.top, self.inputs)

    def append_bottom_from_inputs(self, caffe_layer):
        self.append_repeated_blob_names(caffe_layer.bottom, self.inputs)

    def append_top_from_outputs(self, caffe_layer):
        self.append_repeated_blob_names(caffe_layer.top, self.outputs)

    def append_repeated_blob_names(self, caffe_layer_field, blobs):
        if blobs is not None:
            if isinstance(blobs, list):
                caffe_layer_field.extend([blob.name for blob in blobs])
            else:
                caffe_layer_field.append(blobs.name)

    def form_caffe_prototxt(self, prototxt):
        pass
