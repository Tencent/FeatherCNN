...
def cross_entropy_seq(logits, target_seqs, batch_size=None):  #, batch_size=1, num_steps=None):
    ...
    softmax internally. Normally be used for fixed length RNN outputs, see `PTB example <https://github.com/itl/itl/blob/master/example/tutorial_ptb_lstm_state_is_tuple.py>`__.
    ...
    Examples
    --------
    >>> see `PTB example <https://github.com/itl/itl/blob/master/example/tutorial_ptb_lstm_state_is_tuple.py>`__.for more details
