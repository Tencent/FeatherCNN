...
{
public:
	CaffeModelWeightsConvert(std::string &caffe_prototxt_name, std::string& caffe_model_name);
    bool Convert();
...
private :
    ...
    bool save_with_header;
    NetParameter net_param;
	NetParameter net_param_prototxt; 
};

CaffeModelWeightsConvert::CaffeModelWeightsConvert(std::string &caffe_prototxt_name, std::string& caffe_model_name)
{
...
void CaffeModelWeightsConvert::SaveModelWeights()
{
	//Writer
	{
		...
		for (int i = 0; i != net_param_prototxt.layer_size(); ++i)
		{
			...
			for (int j = 0; j != caffe_model_layer.blobs_size(); ++j)
			{
				...
				else
				{
					...
					{
						fprintf(stderr, "Unsupported dimension with dim size %ld\n", caffe_blob.shape().dim_size());
					}
				}
			...
			{
				printf("++++Scale param+++++\n");
				//MobileNet caffemodel has wrong bias term.
				//load from prototxt.
				//auto caffe_scale_param = net_param_prototxt.layer(i).scale_param();
				//printf("caffe scale bias term %d\n", caffe_scale_param.bias_term() ? 1 : 0);
		...
		//Writer
		FILE *netfp = NULL;
		netfp = fopen("test.feathermodel", "wb");
	...
	//Loader
	{
		printf("++++++Start Loader++++++\n");
		FILE *netfp = NULL;
		netfp = fopen("test.feathermodel", "rb");
		...
		fread(net_buffer_pointer, sizeof(uint8_t), file_size, netfp);
		fclose(netfp);
...
int main(int argc, char *argv[])
{
	if (argc < 3 || argc > 4)
	{
        std::cout << "Usage: ./caffe_model_convert $1(caffe_model_name) $2(save_model_name) $3(save_with_header?, value is 1/0, optional)" << std::endl;
        return -1;
    }
	std::string caffe_prototxt_name = argv[1];
    std::string caffe_model_name = argv[2];
    //std::string save_model_name = argv[2];
    CaffeModelWeightsConvert convert(caffe_prototxt_name, caffe_model_name);
    convert.Convert();
